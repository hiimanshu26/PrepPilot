    ...
    return (
        <div className="p-6 max-w-3xl mx-auto">
            <div className="flex items-center justify-between gap-4 flex-wrap">
                <h1 className="text-2xl font-semibold">Interview Report</h1>
                <a className="underline opacity-80" href="/past-interviews">Back</a>
            </div>

            <div className="mt-4 p-5 rounded-2xl border">
                <div className="font-semibold">{data.role} • {data.level}</div>
                <div className="text-sm opacity-70 mt-1">{data.interviewType}</div>
                <div className="text-sm opacity-70 mt-1">Score: {data.summary?.score ?? "—"}/100</div>
                ...
                <div className="mt-4 grid gap-4">
                    <div>
                        ...
                        <ul className="list-disc pl-6 mt-2 opacity-90">
                            {(data.summary?.topStrengths || []).map((s: string, i: number) => <li key={i}>{s}</li>)}
                        </ul>
                    </div>
                    ...
                    <div>
                        ...
                        <ul className="list-disc pl-6 mt-2 opacity-90">
                            {(data.summary?.topImprovements || []).map((s: string, i: number) => <li key={i}>{s}</li>)}
                        </ul>
                    </div>
                </div>
            </div>
                            ...
                            {fb && (
                                <div className="mt-5 grid gap-4">
                                    ...
                                    <div>
                                        ...
                                        <ul className="list-disc pl-6 mt-2 opacity-90">
                                            {(fb.improvements || []).map((s: string, idx: number) => <li key={idx}>{s}</li>)}
                                        </ul>
                                    </div>
                                    <div>
                                        <div className="font-medium">✨ Better Sample</div>
                                        <ul className="list-disc pl-6 mt-2 opacity-90">
                                            {(fb.betterAnswerSample || []).map((s: string, idx: number) => <li key={idx}>{s}</li>)}
                                        </ul>
                                    </div>
                                </div>
                            )}
